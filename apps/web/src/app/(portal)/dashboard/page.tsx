...
interface Stats {
  ...
  classCount: number;
  /**
   * Dropped entirely by the API for anyone without `fees.view` — the dashboard is where every
   * role lands, so it gates the figures rather than the page. Absent, not zero: a teacher must
   * not be shown a 0% collection rate for a school that is collecting fine.
   */
...
export default async function DashboardPage() {
  ...
  // Carry the percentage on the object so it narrows with it — there is no fee percentage to
  // speak of for a reader who is not shown the fees.
  const fees = stats.fees && { ...stats.fees, pct: Math.round(stats.fees.rate * 100) };
        ...
        {fees && (
          <section className="card p-6 rise rise-3">
            ...
            <div
              ...
              role="img"
              aria-label={`${fees.pct}% collected`}
            >
              <div
                className="h-full rounded-full kente-stripe"
                style={{ width: `${Math.min(100, fees.pct)}%` }}
              />
            </div>
