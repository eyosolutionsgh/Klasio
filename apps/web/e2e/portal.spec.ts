...
test.describe('EYO SMS portal — end-to-end', () => {
  ...
  test('students list, filter and detail with ledger', async ({ page }) => {
    // A bursar holds neither `students.guardians` nor `pickup.view`, so the API omits every
    // guardian's phone and custody flag from this payload. Read as a crash rather than a gap,
    // that took down the whole page behind the error boundary — and a client-side throw leaves
    // nothing in the server log, so watch for it directly.
    ...
    // Headings, not free text: the Concessions blurb also says "fee ledger", so getByText is a
    // strict-mode violation on any student who has a concession — which student this test opens
    // depends on the seed, so that failure comes and goes.
    await expect(page.getByRole('heading', { name: 'Fee ledger' })).toBeVisible();
    // The guardians section still renders — minus the phone, custody badge and edit controls
    // this role may not have.
    ...
    expect(clientErrors, 'student detail threw in the browser').toEqual([]);
  });
