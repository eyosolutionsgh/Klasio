    ...
    <main className="p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      <header className="mt-3 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-oat font-mono">{client.slug}</p>
        </div>
        {note && <p className="text-sm text-clay max-w-sm">{note}</p>}
      </header>
      ...
      */}
      <section className="card mt-6 p-6">
        ...
        {beat ? (
          <dl className="mt-4 grid sm:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <dd className="mt-0.5 tabular-nums">
                {beat.students ?? '—'}
                {beat.studentCap != null && <span className="text-oat"> / {beat.studentCap}</span>}
              </dd>
            </div>
            <div>
          ...
          <p className="text-sm text-oat mt-2">
            Nothing yet. Either reporting is switched off on their server, or it cannot reach us —
            neither stops the school working.
          </p>
        )}
          ...
          <details className="mt-4">
            ...
            <ul className="mt-2 text-xs text-oat space-y-1 font-mono">
              {client.heartbeats.map((h) => (
                <li key={h.id}>
                  {when(h.receivedAt)} · {h.state} · {h.tierInForce} · {h.students ?? '—'} students
                  · {h.verifiedWith}
                </li>
              ))}
            </ul>
      ...
      <section className="card mt-6 p-6">
        ...
        {client.licences.length === 0 ? (
          <p className="text-sm text-oat mt-2">None yet.</p>
        ) : (
          <ul className="mt-4 space-y-3">
            {client.licences.map((l) => (
              <li key={l.id} className="border border-mist rounded p-4">
                ...
                {l.issuedBy && (
                  <p className="text-xs text-oat mt-0.5">Issued by {l.issuedBy.name}</p>
                )}
                {/*
      ...
      {canIssue() ? (
        ...
        <p className="card mt-6 p-4 text-sm text-clay">
          No signing key on this server, so nothing can be issued from here. The CLI still works on
          a machine that has the key: <code>pnpm --filter @eyo/api licence:mint</code>.
        </p>
      )}
    </main>
  );
