...
1. **Below JHS, the class teacher is the school.** Creche → B6 has one teacher *stationed* to a
   ...
   must treat "class teacher" and "subject teacher" as *assignments*, not job titles: the same
   person is often both.
2. **Terms and academic years are lifecycle objects.** A term is *opened*, runs, and is *closed*
...
| F3 | **IT administrator** | Yes (ready-made role) | Runs the box (backups, licence install, users, integrations) **without seeing a child's record or a single cedi**. |
...
- **NaCCA** — curriculum and grading conformance (proficiency bands: *Emerging / Developing / Approaching Proficiency / Proficient / Highly Proficient* — note: not "Beginning/Exceeding").
...
Format: *As a ⟨user⟩, I ⟨do X⟩* — grouped by rhythm. Stories marked ◆ are supported by shipped or
...
- ◆ End: I get the results notification, read the published terminal report on the portal or WhatsApp PDF, download receipts, and see the *next term begins* date printed on the report.
...
**Monthly**
- ◆ I review the collection summary — billed, collected, outstanding, percentage — and the defaulter list (I can *see* money; I cannot *take* it).
...
- ◆ I reconcile the admission register (the permanent numbered ledger NaSIA inspects) — Klasio *is* that register.
...
- ◆ I force a password change when a laptop goes missing; that signs the person out everywhere.
...
6. **Exam-series-that-are-not-terms** — mock 1..n with BECE-style aggregates (4 cores + best 2 electives), mock fees as fee items. (D1) *(partially served by CBT/mocks, Advanced)*
...
15. **NaCCA band naming check** — ensure shipped proficiency-band defaults read *Emerging / Developing / Approaching Proficiency / Proficient / Highly Proficient*.
